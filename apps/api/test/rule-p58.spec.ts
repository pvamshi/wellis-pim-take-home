...
import { p58 } from '../src/rules/catalogue/p58';
import { createRuleContext } from '../src/rules/rule-context';
import type { RuleContext } from '../src/rules/rule-contract';
import { createTemporaryDatabase, type TemporaryDatabase } from './temp-database';

describe('P58 — a patient signup date that is US-style with a certain order', () => {
  ...
  beforeAll(async () => {
    ...
    await patients.save([
      { legacyPatientId: 'p-certain', signupDate: '02/16/2023', rawData: '{}' },
      { legacyPatientId: 'p-both-le12', signupDate: '03/04/2023', rawData: '{}' },
      { legacyPatientId: 'p-dash', signupDate: '14-08-2023', rawData: '{}' },
      { legacyPatientId: 'p-no-month', signupDate: '13/25/1990', rawData: '{}' },
      { legacyPatientId: 'p-no-such-day', signupDate: '02/30/2023', rawData: '{}' },
  ...
  it('proposes the ISO date for a certain-order US-style date, and leaves the rest alone', async () => {
    const response = await p58.run(context);
    ...
    // Both numbers 12 or under (P60's), dash-spelled (P59's), no valid month
    // or day either way, already ISO, and no date at all.
    expect(byId.has('p-both-le12')).toBe(false);
    expect(byId.has('p-dash')).toBe(false);
    ...
    expect(p58.ambiguous).toBe(false);
  });
});
