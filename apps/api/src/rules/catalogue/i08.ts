...
/**
 * I08 — an intake's `submitted_at` whose two leading numbers are both 12 or
 ...
 * and nothing on the row settles it — no other column names a date format or
 * an automation's habit (1.1.12).
 *
 * Boundary: one number above 12 makes the order certain and is I06's fix on
 * the slash spelling, I07's on the dash — this rule is what is left.
 *
 * Tests `submitted_at` and reports against it (1.1.5); an ISO date a human
 * confirms does not match again.
 */
...
export const i08: CatalogueRule = {
  ruleId: 'I08',
  version: 1,
  ruleName: 'Intake submitted date reads as a date both ways round',
  description:
    "This intake's submitted date has two numbers that are both 12 or under, so it is a " +
    ...
    'submission date has to write it in, year-month-day.',
  ambiguous: true,
  ...
  run: async (context: RuleContext): Promise<RuleResponse> => {
    ...
    for (const intake of intakes) {
      ...
      if (!couldBeMonth(first) || !couldBeMonth(second)) continue; // I06's or I07's finding.
