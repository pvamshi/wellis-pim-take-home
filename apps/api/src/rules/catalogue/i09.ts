...
/**
 * I09 — an intake's `submitted_at` that falls after today, so the row claims
 ...
 * (1.1.12).
 *
 * Boundary: reported only when every reading of the cell is future, so I08's
 * unresolved order never decides the finding; an unparseable or empty cell is
 * neither this rule's nor I08's, and I11's respectively.
 *
 * Tests `submitted_at` and reports against it (1.1.5); a real submission
 * date in the past keeps an approved row from matching again.
 */

/** A full ISO date, or two numbers on a slash or a dash and a four-digit
 * year — the shapes I06, I07 and I08 already read on this column. */
...
export const i09: CatalogueRule = {
  ruleId: 'I09',
  version: 1,
  ruleName: 'Intake submitted date is in the future',
  description:
    "This intake's submitted date falls after today, so the row claims a submission that " +
    'has not happened yet. Nothing on the row says what date was meant instead — it could ' +
    'be a mistyped year, a slipped digit, or a value nobody entered on purpose — so no ' +
    'date is proposed here. Someone who can check the real submission date has to write ' +
  ...
  run: async (context: RuleContext): Promise<RuleResponse> => {
    ...
    for (const intake of intakes) {
      ...
      if (previous === null) continue; // I11's finding.

      const readings = readingsOf(previous.trim());
      if (readings.length === 0) continue; // Not a real date in any reading.

      // Some reading is today or earlier — a perfectly good submitted date.
