...
/**
 * M06 — fills a patient's empty `phone` from a confirmed duplicate.
 *
 ...
 * confirmed X's holding different `phone` values propose nothing; X's value
 * is proposed exactly as it stands, even if unformatted — normalising it is
 * P30-P36's job, not this one's (1.1.4), and catches it on the next pass.
 *
...
export const m06: CatalogueRule = {
  ruleId: 'M06',
  version: 1,
  ruleName: "Patient's phone can be filled from a confirmed duplicate",
  description:
    "A confirmed duplicate of this patient holds a phone number and this row's own phone " +
    "is empty. That duplicate's value is proposed, unless more than one confirmed duplicate " +
    'holds a different phone number, in which case nothing is proposed.',
  ambiguous: false,
  ...
  run: async (context: RuleContext): Promise<RuleResponse> => {
    ...
    for (const y of patients) {
      ...
      for (const duplicateRowId of duplicateRowIds) {
        ...
        if (!isEmpty(x.phone)) values.add(x.phone as string);
      }
