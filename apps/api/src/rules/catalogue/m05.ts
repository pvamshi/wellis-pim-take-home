...
/**
 * M05 — fills a patient's empty `bsn` from a confirmed duplicate.
 *
 ...
 * confirmed X's holding different `bsn` values propose nothing; X's value is
 * proposed exactly as it stands — cleaning it up is P26-P29's job, not this
 * one's (1.1.4).
 *
...
export const m05: CatalogueRule = {
  ruleId: 'M05',
  version: 1,
  ruleName: "Patient's BSN can be filled from a confirmed duplicate",
  description:
    "A confirmed duplicate of this patient holds a BSN and this row's own bsn is empty. " +
    "That duplicate's value is proposed, unless more than one confirmed duplicate holds a " +
    'different bsn, in which case nothing is proposed.',
  ambiguous: false,
  ...
  run: async (context: RuleContext): Promise<RuleResponse> => {
    ...
    for (const y of patients) {
      ...
      for (const duplicateRowId of duplicateRowIds) {
        ...
        if (!isEmpty(x.bsn)) values.add(x.bsn as string);
      }
