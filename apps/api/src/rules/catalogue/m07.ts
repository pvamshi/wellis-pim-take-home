...
/**
 * M07 — fills a patient's empty `city` from a confirmed duplicate.
 *
 ...
 * confirmed X's holding different `city` values propose nothing; X's value
 * is proposed exactly as it stands — cleaning it up is P37-P40's job, not
 * this one's (1.1.4).
 *
...
export const m07: CatalogueRule = {
  ruleId: 'M07',
  version: 1,
  ruleName: "Patient's city can be filled from a confirmed duplicate",
  description:
    "A confirmed duplicate of this patient holds a city and this row's own city is empty. " +
    "That duplicate's value is proposed, unless more than one confirmed duplicate holds a " +
    'different city, in which case nothing is proposed.',
  ambiguous: false,
  ...
  run: async (context: RuleContext): Promise<RuleResponse> => {
    ...
    for (const y of patients) {
      ...
      for (const duplicateRowId of duplicateRowIds) {
        ...
        if (!isEmpty(x.city)) values.add(x.city as string);
      }
