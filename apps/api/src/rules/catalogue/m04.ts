...
/**
 * M04 — fills a patient's empty `sex` from a confirmed duplicate.
 *
 ...
 * confirmed X's holding different `sex` values propose nothing; X's value is
 * proposed exactly as it stands — normalising it is P23-P25's job, not this
 * one's (1.1.4).
 *
...
export const m04: CatalogueRule = {
  ruleId: 'M04',
  version: 1,
  ruleName: "Patient's sex can be filled from a confirmed duplicate",
  description:
    "A confirmed duplicate of this patient holds a sex and this row's own sex is empty. " +
    "That duplicate's value is proposed, unless more than one confirmed duplicate holds a " +
    'different sex, in which case nothing is proposed.',
  ambiguous: false,
  ...
  run: async (context: RuleContext): Promise<RuleResponse> => {
    ...
    for (const y of patients) {
      ...
      for (const duplicateRowId of duplicateRowIds) {
        ...
        if (!isEmpty(x.sex)) values.add(x.sex as string);
      }
